...
//ft_strjoin_allocs1 if num = 0, free s2 and s1, if num = 1: free s1
...
char	*ft_copy_char(char *str, int *k, t_cmd *cmd)
{
	...
	i = 0;
	j = 0;
	res = (char *)ft_calloc(ft_strlen(str) + 100, sizeof(char));
	if (!res)
		return (NULL);
	while (str[i])
	{
		tmp = str;
		if (str[i] == '~' && cmd->type[*k] == 'v')
		{
			str = ft_strjoin("$HOME\0", str + 1);
			free(tmp);
		}
		if (str[i] == '$' && (str[i + 1] && cmd->type[*k] != 's'))
			return (ft_copy_char_env(&res, str, &i, cmd), free(tmp), tmp = NULL, res);
		else
			res[j++] = str[i++];
	}
	res[j] = '\0';
